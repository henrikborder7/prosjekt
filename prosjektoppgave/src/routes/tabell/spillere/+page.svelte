...
<script>
  ...
  function sortListByGoals() {
    ...
    rangertListeMål = json.elements.map(
      (spiller) =>
        `${spiller.first_name} ${spiller.second_name}: ${spiller.goals_scored} mål`,
    );
  ...
  function sortListByAssists() {
    ...
    rangertListeAssist = json.elements.map(
      (spiller) =>
        `${spiller.first_name} ${spiller.second_name}: ${spiller.assists} assists`,
    );
